...
aws ec2 run-instances --image-id ami-0557a15b87f6559cf --count 1 --instance-type t2.micro \
    ...
    --tag-specifications \
        "ResourceType=instance,Tags=[{Key=Name,Value=cra_backend}]" \
        "ResourceType=volume,Tags=[{Key=Name,Value=cra_backend_disk1}]"

aws ec2 describe-instances --filters 'Name=tag:Name,Values=cra_backend' \
